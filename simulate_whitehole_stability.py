...
# V5.0 parameters (default)
eps0 = 1.0
R_ast_late = 30.0  # in H0^2 units
M_phi = 1.0  # normalized to M_Pl
c5 = 1.0
m_phi = 1e-33  # ~ H0 in eV units, normalized
Omega_m0 = 0.3
kappa_white_list = [0.0, 1e-10, 1e-9, 1e-8]  # evaporation strengths
...
def R_from_a(a, H0=1.0):
    return 6 * H0**2 * (1.0 / a**3 + 0.7)  # approx for late universe

def Q_white(rho_DE, kappa, f_R_val, H):
    return -kappa * f_R_val * H * np.sqrt(np.abs(rho_DE))  # energy loss from DE

# ODE system (simplified: rho_m, rho_DE)
def deriv(y, a, kappa):
    rho_m, rho_DE = y
    ...
    drho_m = 3*H*rho_m + Q_w  # DE to matter (inverted)
...
def stability_check(kappa, a=1.0):
    H = np.sqrt(8*np.pi*G/3 * (0.3 + 0.7))  # at z=0
    R_val = R_from_a(a)
    f_R_val = f_R(R_val)
    M_eff2 = m_phi**2 + (c5 * f_R_val / M_phi)**2 * kappa**2 * H**2  # approx coupling
    c_s2 = 1 + kappa**2 * (f_R_val / M_phi)**2  # approx
    return M_eff2 > 0 and c_s^2 > 0
